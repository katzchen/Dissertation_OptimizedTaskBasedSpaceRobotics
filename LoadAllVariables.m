function AllVariables = LoadAllVariables()
...
AllVariables.Task.task_start= [-2 8 0 ]; %of the main payload relative to global;
AllVariables.Task.task_end= [2 8 0 ];
...
AllVariables.Arm(1).xyz_base =  [-2 0 0]; %base of arm 1
AllVariables.Arm(1).ArmLength= 15.2*[.5 .5 .1];% .1 .6 .4];
AllVariables.Arm(1).Armoffset= [0 0 0];% .6 .4];
...
AllVariables.Arm(1).JointLimits = [pi/2 2*pi;-2*pi 2*pi; -2*pi 2*pi];% -2*pi 2*pi;-2*pi 2*pi;-2*pi 2*pi];
AllVariables.Arm(1).name ='arm1';
AllVariables.Arm(1).linkcolor =[0 1 0]/2;
...
AllVariables.Arm(1).TrajGenAngle=[pi    2.1496+pi/2    3.1152+pi]; %gives an initial guess for the trajectory generation - mostly used for more complex segments
AllVariables.Arm(1).armtype = 1; %1 = regular, 2 = sRMS

AllVariables.Arm(2).xyz_base =  [2 0 0]; %base of arm 2
AllVariables.Arm(2).ArmLength= 15.2*[.5 .5 .1];% .6 .4];
AllVariables.Arm(2).Armoffset= [0 0 0];% .6 .4];
...
AllVariables.Arm(2).linkcolor =[1 0 1]/2;
...
AllVariables.Arm(2).TrajGenAngle=[  0 0 0 ]; %gives an initial guess for the trajectory generation - mostly used for more complex segments
AllVariables.Arm(2).armtype = 1; %1 = regular, 2 = sRMS
